                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="location_id[]" id="location_id" multiple="multiple"
                                            class="form-control select2 custom-select">
                                            @foreach ($locations as $location)
                                                {{-- <option @if ($job && in_array $job->location_id == $location->id) selected @endif
                                                    value="{{ $location->id }}">
                                                    {{ ucfirst($location->location) . ' (' . $location->country->country_code . ')' }}
                                                </option> --}}
                                                <option @if($job && in_array($location->id , $jobLocation)) selected @endif value="{{ $location->id }}">{{ ucfirst($location->location). ' ('.$location->country->country_code.')' }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
